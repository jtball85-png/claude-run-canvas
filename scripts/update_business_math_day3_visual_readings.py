# -*- coding: utf-8 -*-
"""One-off targeted update of Business Math (course 456) Day 3 reading pages,
per Josh's feedback (2026-09-09): same visual rebuild as Days 1-2, applied
to the 4 Unit 2 (Decimals, Part 1) reading pages -- place value, reading/
writing, comparing, and rounding decimals. Only these 4 pages' body HTML
changes -- Practice 12-15 quizzes and Day 3 Overview/Want More Practice?
are untouched.

Source of truth for the new bodies is the sibling repo's
day3-module-fragment.json (Master Business Finance Program), read directly
here so nothing gets retyped/out of sync.

Uses targeted PUTs against already-live Pages (matched by url slug), same
pattern as the Day 1/2 update scripts -- never push_course.py's
...
FRAGMENT_PATH = (
    ...
    r"\day3-module-fragment.json"
)

SLUG_BY_TITLE = {
    "Place Value in Decimals — Reading": "place-value-in-decimals-reading",
    "Reading and Writing Decimals — Reading": "reading-and-writing-decimals-reading",
    "Comparing Decimals — Reading": "comparing-decimals-reading",
    "Rounding Decimals — Reading": "rounding-decimals-reading",
}
...
def normalize(html):
    ...
    # unicode chars, and boolean attributes like `allowfullscreen` gain an
    # explicit `=""`. Normalize before comparing so a clean round-trip reads
    ...
    # Canvas's editor normalizes tables by inserting an implicit <tbody>.
